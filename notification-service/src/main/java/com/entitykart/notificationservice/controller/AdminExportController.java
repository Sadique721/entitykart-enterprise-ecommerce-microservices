...
import com.entitykart.notificationservice.service.EmailService;
...
public class AdminExportController {
    ...
    // Helper methods to resolve types safely
    ...
    public void exportProductsToExcel(HttpServletResponse response) throws IOException {
        ...
        String[] cols = {"Product ID", "Name", "Brand", "Description", "Price", "MRP", "Stock", "SKU", "Category ID", "SubCategory ID", "Seller ID", "Created At", "Discount %"};
        createHeaderRow(sheet, cols);
    ...
    public void exportReturnsToExcel(HttpServletResponse response) throws IOException {
        ...
        String[] cols = {"Return ID", "Order ID", "Customer ID", "Product ID", "Quantity", "Reason", "Status", "Refund Amount", "Admin Note", "Rejection Reason", "Created At"};
        createHeaderRow(sheet, cols);
        ...
        try (PrintWriter writer = response.getWriter()) {
            ...
            for (Map<String, Object> rt : list) {
                writer.printf("Return #%d | Order #%d | Status %s | Reason %s | Refund ₹%.2f%n",
                        getLong(rt, "returnId"), getLong(rt, "orderId"), getStr(rt, "status"), getStr(rt, "reason"), getDouble(rt, "refundAmount"));
            }
        }
    }
        ...
        try (PrintWriter writer = response.getWriter()) {
            ...
            for (Map<String, Object> w : list) {
                writer.printf("Wishlist #%d | Product #%d | Name %s | Price ₹%.2f | Added %s%n",
                        getLong(w, "wishlistId"), getLong(w, "productId"), getStr(w, "productName"), getDouble(w, "price"), getStr(w, "addedAt"));
            }
        }
    }
    ...
    private void autoSizeColumns(Sheet sheet, int columnCount) {
        for (int i = 0; i < columnCount; i++) {
            try {
                ...
                // Ignore failure if fonts are missing in container environment
            }
        }
    }
        ...
        try (Workbook wb = new XSSFWorkbook()) {
            ...
            if ("orders".equalsIgnoreCase(reportType)) {
                ...
                for (Map<String, Object> p : list) {
                    ...
                    row.createCell(1).setCellValue(getStr(p, "productName"));
                    row.createCell(2).setCellValue(getStr(p, "brand"));
                    row.createCell(3).setCellValue(getDouble(p, "price"));
                    row.createCell(4).setCellValue(getLong(p, "stockQuantity"));
                    row.createCell(5).setCellValue(getDouble(p, "discountPercent"));
                }
